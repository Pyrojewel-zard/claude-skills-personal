...
print("Fetching all items in '导入' collection...")
...
if not collection_key:
    print("Cannot find '导入' collection")
    exit(1)

print(f"'导入' collection key: {collection_key}")

# 分页获取所有条目
print("\nFetching all items in collection...")
items = []
start = 0
limit = 100
...
print(f"\nItems without DOI: {len(no_doi_items)}")

# 保存到文件
with open('/tmp/remaining_no_doi_items.json', 'w', encoding='utf-8') as f:
    json.dump(no_doi_items, f, ensure_ascii=False, indent=2)

print(f"\nRemaining items saved to /tmp/remaining_no_doi_items.json")
print("\nRemaining items:")
for item in no_doi_items:
    print(f"{item['key']}: {item['title'][:80]}...")
