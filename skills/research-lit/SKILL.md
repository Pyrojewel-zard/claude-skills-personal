...
allowed-tools: Bash(*), Read, Glob, Grep, WebSearch, WebFetch, Write, Agent, mcp__zotero__*, mcp__obsidian-mcp-tools__search_vault_smart
---
...
| 2 | **Obsidian** (via MCP) | `obsidian` | Call `mcp__obsidian-mcp-tools__search_vault_smart` — if unavailable, skip | Research notes, paper summaries, tagged references, wikilinks |
...
**Collection handling via /paper:**

For each highly relevant paper found in Zotero, invoke `/paper` to handle collection assignment:

```
/paper "[paper title]"
```

This triggers `/paper` Stage 1 which:
- Creates raw notes in `raw/notes/papers/{paper_id}.md`
- Suggests appropriate Zotero collection based on L2 mapping
- Lets user confirm collection assignment
- Updates tags (`_Read`, `wiki-ingested`, etc.)

**Batch mode:** If multiple papers are found, list them with suggested collections and let user confirm in batch:

```
📚 Found N relevant papers. Collection suggestions:

| # | Title | Suggested Collection |
|---|-------|---------------------|
| 1 | [title 1] | lna-design |
| 2 | [title 2] | rf-amplifier |

Confirm all? (y/all/1,2/skip)
```
