...
|----------|----------|
| `raw/` 文件 | 直接读取并编译 |
| URL / DOI | 用 `WebFetch` 获取内容 → 落入 `raw/` → 再编译；WebFetch 失败（404/paywall）→ 提示用户手动下载到 raw/ |
| PDF / Markdown | 转换后编译 |
| 目录 | 逐文件处理 |
| 路径不精确 | 先 `Glob` 模糊搜索 `raw/**/*<basename>*`；多匹配时列出候选请用户选择 |
...
| `uncertain` | 30-70% 重叠，无法确定 | 默认 update + 标记 `[!需人工审核]` |
...
sources: ["source:xxx"]
```
- **必须包含以下结构段**（确保达到 100 行下限）：
  1. 定义与一句话概述（5-10行）
  2. 核心特征表格或列表（15-25行）
  3. 使用场景与适用范围（15-25行）
  4. 与其他 entity/concept 的 `[[双链]]` 关系说明（15-25行）
  5. 局限与风险（10-20行）
  6. 参考来源（5-10行）
- 长度约束 100-300 行；不足 100 行时补充结构段而非堆砌文字
...
```
- **必须包含以下结构段**（确保达到 50 行下限）：
  1. 核心思想一句话定义（5-10行）
  2. 从哪些 entity 提炼（10-15行，含 `[[双链]]`）
  3. 适用范围与边界（10-15行）
  4. 与相关 concept 的对比或互补（10-15行）
  5. 对主线的启示或风险（5-10行）
  6. 参考来源（5行）
- 长度约束 50-200 行
...
| `Glob` | 路径模糊搜索 | Phase 1 Step 1（路径不精确时） |
| `WebFetch` | URL/DOI 内容获取 | Phase 1 Step 1（URL 输入时） |
